...
export function ActivityReceipt() {
  ...
  const copy = getUiCopy(state.personalization.language);

  useEffect(() => {
    if (latest) setVisible(true);
  }, [latest?.sequence]);
  ...
  const activityDescription = `${
    latest.origin === 'agent' ? copy.agentChanged : copy.youChanged
  } ${copy.activityActions[latest.action]}`;
    ...
    <>
      ...
      <div className="activity-receipt">
        ...
        <summary>
          ...
          <span>{copy.activityActions[latest.action]}</span>
        </summary>
        <div className="activity-detail">
          <p>
            {copy.revisionLabel} {latest.fromRevision} → {latest.toRevision}
          </p>
          {state.undoSnapshot ? (
            <button
              type="button"
              className="activity-undo"
              onClick={() => controller.undoLastChange('human', state.revision)}
            >
              {copy.undoLastChange}
            </button>
          ) : null}
        ...
        <button
          ...
          aria-label={copy.dismissActivity}
